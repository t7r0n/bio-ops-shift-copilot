...
COMPANY = 'Nyad'
REPO = 'bio-ops-shift-copilot'
PROJECT_TERMS = ['wastewater', 'operators', 'reason', 'biological', 'process', 'state', 'pretending', 'sensor-free']
PROJECT_METRICS = ['wastewater_coverage', 'operators_risk', 'reason_precision', 'biological_latency']
PROJECT_FAILURES = ['wastewater_drift', 'operators_gap', 'reason_misroute', 'biological_blindspot']
PROJECT_ARCHETYPES = [{'name': 'wastewater evidence replay', 'trigger': 'wastewater signal changes while operators context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'operators boundary probe', 'trigger': 'operators handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'reason regression harness', 'trigger': 'reason behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'biological operator packet', 'trigger': 'biological output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A shift-level wastewater operations copilot that turns synthetic microscopy/process logs into explainable biological state, confidence bands, and operator handoff notes.'
...
HERO_TITLE = 'Nyad Evidence Triage Board'
